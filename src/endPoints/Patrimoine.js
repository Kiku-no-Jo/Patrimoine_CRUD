      ...
      while (currentDate <= possessionEndDate) {
        if (currentDate >= start && currentDate <= end) {
          ...
          const monthsElapsed = Math.floor(
            (currentDate - possessionStartDate) / (1000 * 60 * 60 * 24 * 30)
          );
          const amortizedValue =
            currentValue * Math.pow(1 - amortRate, monthsElapsed);
    ...
    if (specificDate) {
      ...
      const totalAtDate = possessions.reduce((sum, p) => {
        ...
        if (date >= possessionStartDate && date <= possessionEndDate) {
          const monthsElapsed = Math.floor(
            (date - possessionStartDate) / (1000 * 60 * 60 * 24 * 30)
          );
          const amortRate = p.taux ? p.taux / 100 : 0;
          const amortizedValue =
            p.valeur * Math.pow(1 - amortRate, monthsElapsed);
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="total-display mt-3">
        <h4>
          Total des Patrimoines à la Date Sélectionnée: {total.toFixed(2)} Ar
        </h4>
      </div>
        ...
        <tbody>
          {filteredPossessions.map((p) => (
            <tr key={p.libelle}>
              ...
              <td>
                {p.dateFin ? new Date(p.dateFin).toLocaleDateString() : "N/A"}
              </td>
